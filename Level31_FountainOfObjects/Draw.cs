...
internal class Draw : IDraw
{
    ...
    internal Draw(MainRoom? mainRoom, Player player)
    {
        _mainRoom = mainRoom;
        _player = player;
        _subRooms = new List<SubRoom> { new FountainRoom(_mainRoom) };
    }
    ...
    private void SpriteDrawOrder(int i, int j)
    {
        ...
        if (DrawFountainLocation(ConsoleColor.DarkCyan, '#')) return;
        if (DrawSense(_subRooms?[0].SeeingCoords, ConsoleColor.Blue, '!')) return;
        //if (_subRooms != null && DrawSense(_subRooms[0].SmellingCoords, ConsoleColor.Black, '~')) return;
    ...
    {
        if (_player.Position != null && _drawRowColumn != null &&
            (_player.Position.Row != _drawRowColumn.Row ||
    ...
    {
        if (_drawRowColumn != _subRooms?[0].Location) return false;
        WriteResetChar(c, color);
        return true;
    }

    private bool DrawSense(List<IMainRoom.Coordinate>? listSense, ConsoleColor color, char c = '+')
    {
