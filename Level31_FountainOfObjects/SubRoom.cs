...
internal class SubRoom : ISubRoom
{
    ...
    void ISubRoom.SubRoomCoordinates(IMainRoom.Coordinate ijCoord, IMainRoom.Coordinate? ijCoordTarget)
    {
        ...
        SenseCoordinateAdjacent(ijCoord, ijCoordTarget, SenseTypes.Smell, SeeingCoords);
    }

    protected void SenseCoordinate(IMainRoom.Coordinate coord, IMainRoom.Coordinate? coordTarget, int rowDist,
        int colDist, SenseTypes senseType, List<IMainRoom.Coordinate>? sense)
    {
        if (coordTarget != null && (coordTarget.Row > rowDist || coordTarget.Column > colDist)) return;
        SenseCoords[coord.Row, coord.Column] = senseType;
        sense?.Add(item: new IMainRoom.Coordinate(coord.Row, coord.Column));
    }
